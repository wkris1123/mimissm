...
import com.wk.utils.FileNameUtil;
...
/**
 * @className: com.wk.controller.ProductInfoDo
 * @description: TODO
 * @author: 酷盖i
 * @create: 2022-02-22 13:19
 */
@RequestMapping("/prod")
@Controller
public class ProductInfoDo {
    ...
    public void ajaxSplit(ProductInfoVo vo, HttpSession session){   //这里用session是因为没用到转发，数据不共享
        //取得当前page参数的页面的数据
    ...
    //多条件查询功能的实现
    @ResponseBody
    @RequestMapping(value = "/condition.do",method = RequestMethod.POST)
    public void condition(ProductInfoVo vo, HttpSession session){
        List<ProductInfo> listInfo=productInfoService.selectCondition(vo);
        session.setAttribute("list",listInfo);
    }

    //异步ajax文件上传处理
    @ResponseBody
    @RequestMapping("/ajaxImg.do")
    ...
    public String save(ProductInfo info,HttpServletRequest request){
        ...
        //info有表单提交上来的5个数据，有异步ajax上来的图片名称数据，有上架时间的数据
        int num=-1;
        try {
        ...
        //清空saveFileName变量中的内容，为了后续使用
        saveFileName="";

        //增加成功后应该重新访问数据库，所以跳转到分页显示的do上
    ...
    public String update(ProductInfo info,HttpServletRequest request){
        //因为ajax的异步图片上传，如果上传过，则saveFileName李有上传上来的图片名称，如果没有使用异步ajax上传过图片，则saveFileName=”“;
        //实体类info使用隐藏域表单提供上来的pImage原始图片的名称
    ...
    public String deletePro(int pid,ProductInfoVo vo,HttpServletRequest request){
        int num=-1;
        try {
